...
if formulaire.getvalue('pseudo') != None and formulaire.getvalue('mdp') != None:	#Si l'ont reçoit un pseudo et mot de passe
	LdapMod=Utils.ldapp.LdapTest()							#On charge la classe LdapTest pour manipuler la base ldap
	sha=hashlib.sha1()								#On charge sha1 qui nous permettra de coder le mot de passe
	user=cgi.escape(formulaire.getvalue('pseudo'))
	...
	if(verification==True):								#Si verification vaut True, cela veut dire que les mots de passe sont identiques
		...
		secretary=LdapMod.Search(dn, searchScope, retrieveAttributes, searchFilter, False, False)	#Je fait une recherche dans l'annuaire afin de récuperé l'iut ratacher
