...
if formulaire.getvalue("type")!=None:						#On vérifie que l'ont à bien reçu des infomrations
	#Si l'ont n'a detecté un argument dans type on vas regarder à quoi il correspond
	sqlDB=Utils.SQLTest("concours")

	if formulaire.getvalue("type")=="delenigme" and formulaire.getvalue("id")!=None: 	#Si le type vaut delenigme et que l'ont à reçu un id correspondant à l'énigme à supprimée
		...
		os.chdir("/var/www/UnityScript/concours")			#On se place dans le répertoire de la ou son enregistrées chaques énigmes afn de se préparer à supprimer la répertoire de l'énigme s'il existe
		...
		try:
			...
			employe=cgi.escape(formulaire.getvalue("employeType"))
        		mdp=Utils.password.Pass().GeneratePass(mail)		#On génére un mot de pase puis on l'nevoie à l'adresse mail de l'utiliateur(mot de passe qui servirat à l'utilisateur d'enregistrez son propre mot de passe
			sha.update(mdp)
			mdp=sha.hexdigest()
		...
		prenom=cgi.escape(formulaire.getvalue("user"))			#On récupére le pseudo du membre à supprimé
		...
		if type(members) is list:
			for m in members:					#On ajoute l'nensemble des membres de l'équipe dans un tableau
				memberList.append(m)
		else:
		...
		sqlDB.DelEntry("categorie","IDcat",id)					#On suppime notre catégorie grâce à son ID et la fonction DelEntry
