...
class LdapTest:
	...
	def Search(self, baseDN, searchScope, retrieveAttributes, searchFilter, MultResult, returnDN):
		...
		result_set = []
		if MultResult == False:																	#On regarde si l'on souhaite plusieurs ou un seul résultat en retour
			...
			if len(result_set) > 0:																#On vérifie que l'on à bien un résultat
				if returnDN==False:																#On vérifie si l'on souhaite retourner ou non le DN de la réponse
					resultat=result_set[0][0][1]												#La réponse se trouvant sous forme de tableau, on séléctione uniquement la réponse
					if len(retrieveAttributes)==1:												#On vérifie le nombre d'attribut de recherche
						...
						resultat2=[]
						for attr in retrieveAttributes:											#Si l'on à plusieurs attribue on va alors réaliser une boucle sur chaques attributs
				...
				return "Pas de resultat"														#Si l'on à pas de résultat ont retourne "Pas de resultat"
		else:
			...
			if len(result_set) > 0:
				...
				for i in result_set[0]:															#On fait une boucle sur chaque résultat
					...
					for dico in r:																#On parcours chaque élément de la réponse
						if retrieveAttributes.count(dico)!=0:									#Si l'on à un attribut de recherche présent dans la réponse
				...
				return "Pas de resultat"														#Si l'on à pas de resultat à la recherche on retourne "Pas de resultat"
	...
	def AddMember(self, cn, sn, mail, DNiut, employee, password):
		...
		dn="cn="+cn+",ou=people,o=concours"														#On reconstitue le dn de l'utilisateur à ajouter
	...
	def AddGroup(self, teamName, members, dnIUT, owner):
		...
		attrs = {}																				#On crée le dictionnaire qui va contenir les info de l'équipe
	...
	def AddScore(self, value, DNteam):
		...
		oldscore = self.Search(DNteam, searchScope, retrieveAttributes, searchFilter, False, False)	#On fait une recherche pour récuperer uniquement le score de l'équipe concernée
		newscore = int(oldscore)+value															#On ajoute au nombre de point de l'équipe la valeur souhaitée
